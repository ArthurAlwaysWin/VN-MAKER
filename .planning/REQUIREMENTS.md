...
- [x] **DATA-03**: 运行时变量变化与解锁变化统一走最小 effect DSL（至少支持 `var:set`、`var:add`、`var:sub`、`unlock:ending`、`unlock:cg`）

### Variable Authoring

- [x] **VAR-01**: 用户可以在项目级变量注册表中创建并管理变量，包含 `id`、显示名、类型（bool/number）、默认值、分组、备注
...
- [x] **BRN-01**: 用户可以通过条件页 GUI 配置 1~3 条变量条件，支持 `== != > >= < <=` 与“全部满足 / 任一满足”
...
| VAR-01 | Phase 84 | Complete |
| VAR-02 | Phase 84 | Pending |
| VAR-03 | Phase 84 | Pending |
| BRN-01 | Phase 84 | Complete |
