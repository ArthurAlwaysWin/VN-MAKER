...
- [x] **PIPE-01**: 用户可一键导出完整可运行的 Windows 桌面游戏文件夹（含 .exe）
- [x] **PIPE-02**: 导出使用 @electron/packager 集成打包，输出绿色免安装目录
- [x] **PIPE-03**: 导出时完整复制所有游戏资源（图片、音频、字体、语音）到输出目录
- [x] **PIPE-04**: 导出过程显示实时进度（复用 ExportModal 3 态模式：配置→进度→完成）
- [x] **PIPE-05**: 用户可选择导出输出目录位置
- [x] **PIPE-06**: 导出完成后可选 ZIP 压缩输出目录
- [x] **PIPE-07**: Electron 运行时二进制自动缓存，二次导出无需重复下载
...
- [x] **CUSTOM-01**: 用户可自定义游戏标题（窗口标题 + .exe 元数据）
- [x] **CUSTOM-02**: 用户可提供 PNG 图标，自动转换 .ico 并嵌入 .exe
- [x] **CUSTOM-03**: 导出游戏窗口尺寸从项目设置读取（默认 1280×720）

## 导出 UI (Export UI)

- [x] **UI-01**: ExportModal 新增 Web/桌面导出格式切换
- [x] **UI-02**: 桌面导出模式显示图标选择器
