...
`review-handoff` preserves the existing individual `author-check` and `handoff-report` commands for scripts that need separate artifacts. `handoff-report --source-map` is optional P9 metadata: it reads an enriched Agent DSL source map, adds `agent-dsl` review items for generated changes, annotates handoff preview targets with source file/line/mapping ids, and reports stale/missing/untracked generated regions without writing DSL data into `script.json`.
