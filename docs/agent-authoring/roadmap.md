...
- Transition authoring exposes the current shared catalog in page-inspector controls: directional wipe, zoom, flash, iris, crossfade-pan, CSS-only shape/curtain/stylized ids, and the procedural `noise-dissolve`/`ripple`/`burn` canvas-mask ids for background rendering/preview; character fade/slide/pop/scale/blur motion; camera vignette/letterbox overlays; and bounded bulk background operations through `set-page-transitions`.
